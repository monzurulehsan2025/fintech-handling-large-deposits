...
import { MOCK_DEPOSITS, Deposit, PartnerBankAllocation } from '@/lib/mockData';
...
export default function DepositsPage() {
    ...
    const simulateAllocation = () => {
        ...
        const simulated = selectedDeposit.allocations && selectedDeposit.allocations.length > 0
            ? [...selectedDeposit.allocations].reverse()
            : [
                ...
                { partnerBankName: 'Local Federal', allocatedAmount: selectedDeposit.amount - 500000, fdicLimit: 250000, insuredPercent: Math.min(100, (250000 / (selectedDeposit.amount - 500000)) * 100) },
            ];

        const updatedDeposit = { ...selectedDeposit, status: 'Allocated' as const, allocations: simulated };

        setDeposits(prev => prev.map(d => d.id === selectedDeposit.id ? updatedDeposit : d));
        setSelectedDeposit(updatedDeposit);
    };

    const getInsuredTotal = (deposit: Deposit) => {
        return deposit.allocations?.reduce((acc, alloc) => acc + (alloc.allocatedAmount * (alloc.insuredPercent / 100)), 0) || 0;
    };
                                            ...
                                            <table className="w-full text-xs">
                                                ...
                                                <tbody className="divide-y divide-gray-200">
                                                    {deposit.allocations && deposit.allocations.length > 0 ? (
                                                        deposit.allocations.map((alloc, i) => (
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {selectedDeposit.allocations && selectedDeposit.allocations.length > 0 ? (
                                    selectedDeposit.allocations.map((alloc, i) => (
